...
WAL(Write Ahead Logging): 트랜잭션이 커밋되기 전에 로그를 먼저 디스크에 기록하는 방식.  

Redo & Undo 로그는 WAL 전략을 사용하여 원자성과 지속성을 제공한다.  

- **Redo Log**  
  - 트랜잭션이 커밋된 후 시스템 장애가 발생하여 디스크에 저장되지 않았을 때 성공한 트랜잭션의 변경사항을 다시 디스크에 적용하여 복구할 수 있게 해주는 로그.  
  - 커밋된 데이터를 디스크에 안전하게 기록해주는 역할.  
  - 지속성 보장, 일종의 다시하기.  
- **Undo Log**  
  - 트랜잭션 중에 롤백 혹은 오류 발생 시 이전 상태로 되돌릴 수 있게 해주는 로그.  
  - 변경전 데이터를 Undo 로그에 저장, 롤백시 Undo 로그를 사용해 변경된 데이터를 원래 상태로 복구.  
  - 원자성 보장, 일종의 되돌리기.  

MySQL 은 Redo & Undo 로그를 기반으로 트랜잭션 관리.  

> 추가적으로 `MySQL InnoDB` 엔진의 경우 디스크쓰기 작업 오류 방지를 위해 첫 번째로 버퍼에 기록하고, 이후 실제 데이터 파일에 기록하는 `Doublewrite Buffer` 기법을 사용한다.  

PostgreSQL 모두 Redo 로그만 사용하고 롤백은 테이블 버저닝을 사용하여 트랜잭션을 관리.  
...
격리레벨은 커넥션(세션)마다, 트랜잭션마다 고유하게 설정할 수 있으며 아래 명령어로 현재 DB 에서 기본 실행되는 커넥션의 격리레벨 조회가능하다.  
...
- **SERIALIZABLE**  
  읽기 작업에서도 `shared lock` 을 설정, 선점한 트랜잭션이 수행중일 때 `read/write` 할 수 없다.  

위 기준은 `ANSI SQL` 기준이며 DB 제조사별로 격리레벨에 차이가 있다.  

`MySQL` 은 `REPEATABLE_READ` 를 기본레벨로 사용하며 `Dirty Read, Nonrepeatable Read` 와 일부 `Phantom Read` 까지 방지할 수 있다.  
`MySQL` 의 경우 범위 조회 조건이 `Gap Lock` 을 사용할 경우 `Phantom Read` 까지 방지할 수 있다.  

`PostgreSQL` 은 성능을 위해 `READ_COMMITTED` 격리레벨을 기본으로 사용하며 `Dirty Read, Nonrepeatable Read` 까지 방지할 수 있다.  
`PostgreSQL` 의 경우 `REPEATABLE_READ` 레벨 설정시 `MVCC` 의 `Snapshot Isolation` 기반으로 `Phantom Read` 를 방지한다.  
...
트랜잭션은 자신의 `txid` 가 반영된 데이터 레코드만 보이기 때문에 `READ_COMMITTED` 격리레벨만 설정해도 `Dirty Read, Nonrepeatable Read` 가 방지된다. 
`Phantom Read` 까지 방지하려면 `REPEATABLE_READ` 레벨을 설정해야한다.  
