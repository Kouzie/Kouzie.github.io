...
- **비트레이트(bitrate)**  
  특정한 시간 단위(이를테면 초 단위)마다 처리하는 **비트의 수**이다
- **멀티비트 레이트**  
  화질선택 기능을 지원하기 위한 기술, 동영상을 화질별로 세그먼트, 매니페스트로 나누어 클라이언트에게 선택 재생 할 수 있도록 지원한다.  
...
처음 2 개이상의 클라이언트가 연결되기 위해 `시그널링` 이라는 작업을 수행해야 하며 `SDP(Session Description Protocol)` 를 주고받으며 서로의 네트워크 정보를 교환하는데, 이때 `시그널링 서버` 가 필요하다.  

![live-stream6](/assets/2021/live-stream5-1.png)  

시그널링 작업을 통해 각 클라이언트는 상대방의 공인IP 와 오픈된 port 를 알 수 있고 해당 주소로 미디어 데이터를 전송한다.  
WebRTC 자체에는 시그널링 방식이 정의되어 있지 않아서, 개발자가 직접 SDP 를 어떻게 송수신할지 구현해야한다.  

`NAT` 나 방화벽 환경에서 클라이언트 끼리 P2P 전송지원은 방화벽 등에 막힐 수 있어 다른방식으로 `WebRTC` 를 운형해야 하는데  
`STUN`, `TURN` 방식을 사용하여 동작할 수 있도록 한다.  

대부분 현대 네트워크 시스템에선 ISTP 업체를 통해 NAT 기반으로 클라이언트를 구성하기 때문에 STUN 서버는 반드시 사용해야 하며 추가적인 인프라가 필요하다.  
...
대부분 네트워크 환경에서 내부 클라이언트는 아웃바운드 UDP 패킷을 영상 송신자에게 보내 아웃바운드 포트로 영상데이터를 보내도록 유도하여 방화벽 설정 없이 데이터를 수신할 수 있다.  
기본 설정이 stateful 라우터인 경우 큰 문제가 없지만 UDP 패킷을 모두 차단하는 엄격한 기업 방화벽의 경우 `TURN over TCP` 를 사용해야한다.  

> 클라이언트간 P2P TCP 연결은 서버-클라이언트 TCP 연결에 비해 NAT 환경에서 실패할 확률이 높고 거의 사용되지 않아 TCP fallback 시도시 TURN 릴레이 연결을 주로 사용함.

### ICE(Interactive Connectivity Establishment)

`STUN`, `TURN` 등으로 찾아낸 **연결 가능한 네트워크 주소들을 Candidate(후보, 서로 연결할 수 있는 네트워크 경로 정보)로 취급**  

사용가능한 클라이언트의 모든 통신 가능한 주소를 식별한 후 가장 최적의 경로를 찾아서 연결하는 방식이다.  

### WHIP/WHEP/WISH

기존 WebRTC의 웹소켓 기반 복잡한 시그널링을 단순화하기 위한 표준화된 HTTP 기반 프로토콜로 SDP(세션 설명 프로토콜) 를 주고받는 프로토콜

- WHIP (WebRTC-HTTP Ingestion Protocol)
  영상 송신자가 미디어 스트림을 업로드(ingest) 하기위해 시그널링 서버로 HTTP POST 요청을 사용하여 서버에 SDP Offer 전송.  
  - 서버는 201 Created 응답과 함께 SDP answer.
  - ICE 연결이 설정된 후, WebRTC 미디어 스트림이 서버로 전송.
  - 연결을 종료하려면 클라이언트가 HTTP DELETE 요청을 보냅니다.

- WHEP (WebRTC-HTTP Egress Protocol)
  영상 수신자가 미디어 스트림을 다운로드(Egress) 하기 위해 시그널링 서버로 HTTP POST 요청을 사용하여 서버에 SDP Offer 전송.  
  - 클라이언트는 HTTP POST 요청을 통해 를 서버에 전송합니다.  
  - 서버는 201 Created 응답과 함께 SDP answer.
  - ICE 연결이 설정된 후, WebRTC 미디어 스트림이 클라이언트로 전송.
  - 연결을 종료하려면 클라이언트가 HTTP DELETE 요청을 보냅니다.
  
- WISH (WebRTC-HTTP Signaling Protocol)
  - 양방향 WebRTC 통신을 위한 HTTP 기반의 신호 프로토콜  
  - 기본적으로 WHIP/WHEP과 동일한 방식으로 동작하지만, 양방향 미디어 및 데이터 스트림을 지원합니다.
...
SRT, WebRTC 등 대부분의 스트리밍 프로토콜을 지원한다.  

서버 config, 스트림 config 를 파일 또는 API 로 관리할 수 있다. 

> API Doc: <https://bluenviron.github.io/mediamtx/>
...
위 영상에선 `nginx-rtmp` 모듈을 설정해 컴파일, 빌드까지 진행한다.  
`Dockerfile` 를 통해 `nginx` 와 `rtmp` 가 이미 빌드된 도커 이미지를 만들거나  
...
> <https://doc-kurento.readthedocs.io/en/6.18.0/>

![kurento1](/assets/기타/kurento1.png)  

`Kurento Media Server(KMS)` 는 WebRTC 고급 비디오 애플리케이션을 개발하는 데 사용할 수 있는 **멀티미디어 서버** 패키지.  

단순 WebRTC 엔드포인트 시그널링 서버 역할 뿐 아니라 미디어 서버 역할로서, 영상의 변조, 저장, MCU, SFU 등의 역할도 수행한다.  

OS 는 `AWS EC2 Ubuntu 22.04` 사용, 도커기반으로 설치한다.  

> <https://doc-kurento.readthedocs.io/en/6.18.0/user/faq.html?highlight=docker%20cp#kurento-in-docker>
> <https://doc-kurento.readthedocs.io/en/6.18.0/user/installation.html#stun-turn-server-install>

먼저 STUN 서버 역할을 할 `coturn` 을 설치한다.  
...
설치가 완료되면 `/etc/default/coturn` 에서 아래 코드 주석처리를 해제후 서버를 재시작.  
...
STUN 서버가 설치되면 아래 test page 에서 정상동작하는지 테스트 가능.  

> <https://webrtc.github.io/samples/src/content/peerconnection/trickle-ice/>  

![kurento1](/assets/기타/kurento3.png)  

> IPv6 를 지원하지 않아 오류발생, IPv4 는 정상동작함.  

STUN 서버 설치가 완료되었다면 KMS 서버 설치 진행,  
`config` 파일을 `bind` 처리하여 `docker-compose` 로 설치 진행.  

```shell
# config 파일 bind 를 위해 복사
  ...
  kms:
    ...
    volumes:
      - type: bind
        ...
        target: /etc/kurento/
    environment:
      KMS_STUN_IP: 15.XXX.XXX.77
      KMS_STUN_PORT: 3478
```

> <https://doc-kurento.readthedocs.io/en/6.18.0/user/tutorials.html>
> <https://doc-kurento.readthedocs.io/en/6.18.0/tutorials/java/tutorial-one2many.html>
...
git checkout 6.18.0
nohup mvn -U clean spring-boot:run -Dspring-boot.run.jvmArguments="-Dkms.url=ws://localhost:8888/kurento" &
```