...
고품질 테스트 코드, 가치있는 테스트 코드로 **테스트 스위트(Test suite)** 를 만드는 방법을 알아야 한다.  
...
Test suite 의 일관된 패턴 사용함으로서 유지보수가 편해진다.  
...
그런 의미에서 정밀도(Precision) 공식을 사용하여 `Test suite` 를 작성해 나가야 한다.  
