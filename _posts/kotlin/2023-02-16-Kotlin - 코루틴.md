...
하지만 `reactive stream` 의 구현체인 `RxJava`, `Reactor` 와 다르게 **협력형 멀티태스킹** 개념이 있으며  
스레드 변경없이 동시적 코드를 지원하며 (yield, suspend 키워드를 사용하지 않을까)
...
`suspend` 키워드를 사용하는 함수, 비동기함수임을 키워드로 명시할 수 있음  
`suspend` 함수를 맞닥뜨리면 스레드는 작업이 완료될 동안 다른작업을 수행한다  

> 시작하고, 멈추고, 다시 시작할 수 있는 함수, 비동기 실행을 위한 중단 지점의 의미 
> `suspend` 함수는 반드시 코루틴 내부 혹은 다른 `suspend` 함수에서 수행되어야 한다.  

`kotlin` 에서 제공하는 `suspend` 함수로 `delay` 가 있으며  
