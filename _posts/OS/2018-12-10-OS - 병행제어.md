...
이렇게 `S-box` 를 공유하는 `E-box` 가 여러 개인 경우를  
...
아래 그림처럼 while 조건이 충족되면 루프돌기 보다 다른 프로세스에게 CPU를 빨리 넘기는 것이 효율적이다. 
...
`Bounded-Buffer Problem`(유한 버퍼 문제) `Producer-Consumer-Problem`(생산자 소비자 문제) 여러 이름으로 불린다.  
