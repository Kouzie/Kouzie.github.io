...
각 프로젝트별로 k8s helm 기반으로 동작할 수 있도록 문서화가 잘 되어있다.  

설치시 사용한 버전에 따라 설정값이 달라질 수 있지만 각 프로젝트별 컴포넌트를 확인하고 연결해야할 부분만 정확히 파악한다면 설치가능하다.  
...
**write service, backend service** 의 경우 `Ingester` 와 `Compactor` 가 chunk 단위로 로그데이터를 저장하거나 압축하는 과정이 있어 중간에 데이터를 저장하는 단계를 거쳐 stateful 하다.  
...
`[write, backend]` 의 경우 로그를 바로 처리하지 않고 chunk 단위로 처리하다 보니 내부에 일부 로그를 저장해둘 공간이 필요해 stateful 하게 운영된다.  

아래 그림과 같이 `[모노리틱 모드, 마이크로서비스 모드]`로 운영 가능하다.  
...
이번 포스팅에선 `Mimir` 보다 성숙하고 커뮤니티가 활성화된 `Thanos` 설치를 진행한다.  
...
`Prometheus` 또한 파드에서 `Thanos Sidecar` 를 운영해야 하기에 서로 의존관계이다.  

다행히 `Thanos Sidecar` 설치까지 지원하는 helm 차트를 `prometheus-community` 에서 제공한다.  
...
`Prometheus` helm 차트에서도 `Thanos` 를 사용하기 위한 설정 진행.  
...
현재 `bitnami` 에서 배포하는 `Thanos` helm 차트가 가장 인기가 많다.  

> `Thanos` 에서 직접 배포하는 helm 차트는 존재하지 않는다.  
...
`Thanos` 의 helm `value.yaml` 을 보면 대부분의 컴포넌트가 `enabled: false` 처리되어 있다.  

`receiver` 컴포넌트를 제외한 모든 컴포넌트를 `enabled: true` 처리하고,  
...
Grafana helm 차트에서 `Tempo` 의 모놀리식 방식, 마이크로서비스 방식을 모두 지원한다.  
