...
사용자와 데이터 베이스 자원을 효율적으로 관리하기 위한 다양한 정보를 저장하는 시스템 테이블의 집합.
...
측 개체 무결성 규칙은 릴레이션(테이블)에 저장되는 튜플(tuple)의 유일성을 보장하기 위한 제약조건이다.
...
에러가 났다.
이런 경우를 참조 무결성이라 한다.
즉 참조 무결성 규칙은 릴레이션 간의 데이터의 일관성을 보장하기 위한 제약조건이다.
...
도메인 무결성 규칙은 속성에서 허용 가능한 값의 범위를 지정하기 위한 제약조건이다.
...
user_constrains – 뷰(가상테이블) 테이블 이다.
...
table_constraint: 테이블레벨에서 제약조건을 정의하는 경우
CREATE TABLE 테이블명
 (컬럼명 데이터타입,
   ......
  컬럼명 데이터타입,
PRIMARY KEY 생성 	CONSTRAINT constraint명 PRIMARY KEY(컬럼명1,...) 
FOREIGN KEY 생성 	CONSTRAINT constraint명 FOREIGN KEY(컬럼명) REFERENCES 참조테이블명(참조컬럼명) 
UNIQUE CONSTRAINT 생성 	CONSTRAINT constraint명 UNIQUE(컬럼명1,...) 
CHECK CONSTRAINT 생성 	CONSTRAINT constraint명 CHECK(조건)
하나 이상의 제약조건이 두개 이상의 칼럼에 동시에 적용될 때 테이블 레벨에서만 정의 가능

 
column_constraint: 컬럼레벨에서 제약조건을 정의하는 경우
PRIMARY KEY	   컬럼명 데이터타입 CONSTRAINT constraint명 PRIMARY KEY
	[USING INDEX
		STORAGE (STORAGE parameter)
		TABLESPACE index_tablespace명]

UNIQUE	   컬럼명 데이터타입 CONSTRAINT constraint명 UNIQUE
        [USING INDEX
                STORAGE (STORAGE parameter)
                TABLESPACE index_tablespace명]

FOREIGN KEY	   컬럼명 데이터타입 CONSTRAINT constraint명 
	REFERENCES 참조테이블명 (참조컬럼명)

CHECK	   컬럼명 데이터타입 CONSTRAINT constraint명 CHECK (조건)

NOT NULL	   컬럼명 데이터타입 CONSTRAINT constraint명 NOT NULL

	

NOT NULL 제약조건 적용은 컬럼 레벨에서만 정의 가능.
...
NOT NULL제약조건과
PRIMARY KEY 제약조건 2개가 생겼다. 
제약조건명을 붙이지 않으면 SYS_XXXX 형식으로 제약조건명이 자동 설정된다.
...
따라서 ALTER TABLE로 제약조건을 추가 또는 삭제해줘야 한다.
...
NOT NULL은 MODIFY를 사용해서 추가, 삭제해야 한다.
...
CONSTRAINT_TYPE에 NOT NULL은 ‘C’, 기본키는 ‘P’, 외래키는 ‘R’러 표시
...
ON DELETE CASCADE는 10번 소속 레코드를 모두 삭제하는 옵션이고
ON DELETE SET NULL은 10번 소속 레코드의 부서를 모두 NULL로 설정하는 옵션이다.
...
*CASCADE옵션으로 참조하는 칼럼의 제약조건도 비활성화
...
< > =연산자, AND OR연산자 BETWEEN연산자, IN연산자, LIKE연산자 등을 사용해서 해당 컬럼에 저장 가능한 데이터 값의 범위나 조건 지정할 수 있다.
...
보통 복합키는 특별한 상황이 아니면 순번을 기본키로 설정한다, 복합키를 기본키로 하면 성능이 떨어지기 때문...
