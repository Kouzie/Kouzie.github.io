...
오라클 서버가 미국에서 돌아가고 있고 원화표시를 $ 로 표시한다
클라이언트는 한국에서 원을 표시할 때 결국 표시되는 원화는 원이다.
...
MONTHS_BETWEEN 	두 개의 날짜간의 달 차이를 리턴하는 함수 	숫자 
...
SELECT sysdate
 ...
 , ADD_MONTHS(sysdate, 3) AFTER3MONTH
 , ADD_MONTHS(sysdate, -3) BEFORE3MONTH
...
DATE 	date와 time을 저장함 	to_date() 
TIMESTAMP 	DATE 데이터타입의 확장으로, year, month, day, hour, minute, second를 저장 	to_timestamp() 
TIMESTAMP WITH TIME ZONE 	TIMESTAMP에 time zone region name이나 time zone offset을 포함 	to_timestamp_tz() 
...
다양한 날짜타입이 있지만 주로 쓰는 건 DATE… DB에 초 이상의 시간을 기록할 일이 별로 없다.
...
SELECT TO_CHAR(TRUNC(LOCALTIMESTAMP, 'YEAR'), 'YYYY/MM/DD HH24:MI:SS') trunc
FROM dual;
...
날짜형과 날짜형끼리는 무난하게 계산이 되지만 문자형과 날자형과의 연산은 expected CHAR got DATE오류난다.
