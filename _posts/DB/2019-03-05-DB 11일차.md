...
CACHE옵션
한 개씩 뽑는게 아닌 메모리에 미리 여러 개 뽑아 놓는 것. 성능이 증가한다.
...
시퀀스 확인은 user_sequences 딕셔너리에서 확인할 수 있따.
...
sequence는 내부적으로 currval과 nextval이라는 pseudo 컬럼을 사용하여 값을 리턴한다. 
...
currval을 사용하기 위해선 적어도 한번의 nextval를 사용해야 한다. 
currval이 nextval을 참조하기 때문
...
때문에 50, 60을 지우고 다시 시퀀스를 사용해 50, 60번 부서를 INSERT 할 수 없다. 

시퀀스를 삭제하고 START WITH 50 으로 설정해서 다시 만들어서 사용할 수 밖에 없다.
...
만약 UPDATE는 실행됬는데 SELECT가 오류로 인해 실행되지 않았을 경우 조회수를 증가시키지 않아야 한다.

ROLLBACK작업이 이루어 져야 하는데 이런 작업을 트랜잭션 처리 작업이라 한다.
...
하지만 우리 게시판은 비밀번호를 입력해서 확인하는 것만 가능하다.
...
UPDATE tbl_board
set password = ' ', subject = ' ', content= ' ', tag = ' '
WHERE seq = 1
...
INSERT INTO tbl_board
(seq, name, password, email, subject, content, tag, userip)
VALUES(seq_myboard.nextval, dbms_random.string('U', 5)
, '1234$!'
, dbms_random.string('A', 5)||'@origio.net'
, dbms_random.string('A', dbms_random.value(10,20))
, dbms_random.string('A', dbms_random.value(10,20)), 'y', '127.0.0.1');

위의 쿼리를 70개정도 수행
 

75개가 넘는 게시글을 모두 한 화면에 띄울 순 없다.

한페이지에 10개씩 뿌린다 하면 8페이지가 필요할 것이다.

이렇게 하나의 페이지에 10개씩 또는 N개씩 뿌리는 작업을 페이징 처리라 한다.
