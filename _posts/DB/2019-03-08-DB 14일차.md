...
주의할 점은 `ELSEI`F 가 아닌 `ELSIF` 라는 것....  
...
2.OPEN |CURSOR 선언에서 선언된 SELECT문의 실행을 의미
3.FETCH |open된 SELECT문에 의해 검색된 하나의 행 정보를 읽어옴<br>만약, 반환되는 결과가 여러 개인 경우, LOOP...END LOOP와 같은 반복문을 통해 마지막 행이 읽힐 때까지 계속 읽어 들이도록 함
4.CLOSE |선언된 SELECT 문의 선언을 종료함
 
