...
**일관성(Consistency)**  
트랜잭션이 성공적으로 완료하면 언제나 일관성 있는 데이터베이스 상태로 유지하는 것을 의미한다.  
DB 내에 무결성 제약이 모든 계좌는 잔고가 있어야 한다면 이를 위반하는 트랜잭션은 중단된다.
...
아래 그림처럼 스케줄 내에서 `commit` 된 T2 가 `rollback` 된 T2 가 `write` 했던 데이터를 `read` 한 경우를 `Unrecoverability Schedule` 이라 한다.  
...
`Read Lock` 을 흭득하기 위한 `FOR SHARE` 키워드도 존재한다.  

```sql
SELECT balance FROM account WHERE id = '1' FOR SHARE; -- READ LOCK
```

`Read Lock` 의 경우 해당 스레드에선 `SELECT` 요청만이 실행됨을 보장해야 한다.  
만약 `Read Lock` 으로 조회 해놓고 해당 `row` 를 변경하게되면, 타 트랜잭션에서 `Dirty Read - Lost Update` 가 발생할 수 있다.  

### PostgreSQL MVCC 문제점  

`PostgreSQL` 의 `MVCC` 에서도 별도의 `Read Lock` 을 사용하지 않기 때문에 기본 격리레벨인 `READ_COMMITTED` 을 사용할 경우 `Dirty Read-Lost Update` 가 발생한다.  

따라서 `Concurrency Control` 필요한 데이터를 접근할 때에는 격리레벨을 `REPEATABLE_READ` 로 설정해야 한다.  
