...
`READ_COMMITTED` 으록 설정하면 `commit` 된 데이터만 `read` 할 수 있음으로 `Dirty Read` 가 발생하지 않는다.  
...
> `MySQL` 은 기본적으로 `MVCC` 기법을 사용한 `REPEATABLE_READ` 을 사용한다.  
...
`[Account1, Account2]` `Entity` 에 `update 요청` 을 `MVCC` 로 처리하는 과정.  
`txid=12`는 `select 요청` 진행, `txid=13`는 `update 요청` 진행한다.  
...
### 스냅샷 격리(REPEATABLE_READ)  

DB 벤더별로 다르지만 `[PostgreSQL, MySQL]` 에선 `MVCC` 기반의 `REPEATABLE_READ` 를 `스냅샷 격리` 라고 부른다.  

트랜잭션 수행을 형상으로 관리하고 `commit` 시점에 관리하던 형상을 한번에 뭉쳐 `write` 명령을 DB 에 전달해서 `스냅샷 격리` 라 부른다.

`REPEATABLE_READ` 에서 두개의 `write 요청` 트랜잭션에서 `쓰기 충돌` 을 제어 하기위한 여러가지 규칙이 있다.  

1. 트랜잭션이 `abort` 되면 그동안의 `write 명령` 은 모두 무시된다.  
2. 만약 트랜잭션의 `start-commit` 사이에 다른 트랜잭션이 먼저 `commit` 했다면 후에 `commit` 되는 트랜잭션은 `abort` 된다. 그래서 `first-commiter win` 이라는 이름으로 불리기도 한다.  
3. 자신보다 큰 트랜잭션ID 가 작성한 row 는 `commit` 여부 상관없이 질의해도 보이지 않는다.  

`스냅샷 격리` 에서 색인은 모든 `version` 의 row 를 찾고 필터링 하는 방식으로 운영된다.  

### Lost Update 해결방안

`Lost Update` 발생원인은 `read-modify-write` 형태로 구성된 트랜잭션 때문이다.  
...
스냅숏 관리에서 가장 효과적으로 사용될 수 있는 방법이며 어플리케이션에서도 별도의 수정을 할 필요가 없다.  
...
코드 구성에 따라 어플리케이션 레이어에서 제한하고 있는 조건을 위배할 수 있다.  

경비 프로그램에서 최소 한명 이상의 경비원이 모니터링 하고 있고, 각 경비원은 전산에서 휴식버튼을 누르고 휴식할 수 있는 시스템에서 동시에 모든 경비원이 휴식버튼을 누르게 될경우 아마 모두 휴식상태로 값이 변경되며 어플리케이션에서 제한하고 있는 조건을 위배하게 될 것이다.  

경비원 모두 자신의 레코드에 접근하여 status 값을 변경할 것이기 때문에 `Lost Update`, `Dirty Read` 로 발생한 문제도 아니다.  

예약 시스템도 마찬가지이다, row 삽입을 통해 해당 시간대에 예약이 없는지 체크하고 예약 레코드를 삽입하는 시스템에서 동시에 동일한 예약시간에 예약을 진행하면 레코드가 2개가 들어가게 된다.  

이런 이상 현상을 **Write Skew** 라 한다.  
...
2008년에 처음 기술, `낙관적 동시성 제어기법`이라 부르기도 함.  
`SERIALIZABLE` 과 `MVCC`(스냅숏) 을 적절히 섞어서 사용하는 방식이다.  

`SERIALIZABLE SNAPSHOT` 은 잠금 없이 트랜잭션들을 실행시킨다.  
실행중 격리가 되지 않은 트랜잭션은 abort 처리한다, 직렬적으로 실행된 트랜잭션은 `commit` 된다.  

![image1](/assets/DB/mysql/mysql_tx12.png)  

트랜잭션이 직렬적으로 실행되었는지 여부는 `MVCC` 기반의 오래된 `version row` 를 읽었는지 여부로 판단한다.  
트랜잭션이 `commit` 되기 전 해당 트랜잭션이 오래된 `version row` 를 읽었는지 한번 더 체크한다.  

> `MVCC` 기반으로 동작하는 점에서 트랜잭션 관리자의 `갱신손실 자동감지` 와는 다른 개념이라 볼 수 있다.  

`SERIALIZABLE SNAPSHOT` 격리레벨에서 트랜잭션들의 경쟁이 심해지면 직렬로 실행되지 못한 트랜잭션이 많아지면서 abort 비율이 높아지고 되려 `2PL` 보다 못한 성능을 낼 수 있다.  
또한 직렬화 여부를 온전히 DB 알고리즘에 의탁하는 과정에 부하가 일어날 수 있다.  

그럼에도 `SERIALIZABLE SNAPSHOT` 를 사용해야 하는 이유는 락 없이 직렬화 구현이 가능하다는 점, 대부분 질의문이 `read 요청` 이기에 효율적이기 때문이다.  
