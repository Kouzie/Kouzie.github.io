...
`RBO` 의 경우 테이블 사이즈나 분포도를 고려하지 않고 룰 베이스로 실행계획을 수립하기 때문에 항상 동일한 실행계획을 만들어 내지만, 통계정 방식을 같이 사용하는 `CBO` 보다 좋은 성능을 내기 어렵다.  
...
SELECT *
FROM employees e 
    LEFT JOIN LATERAL (SELECT * FROM salaries s WHERE s.emp_no=e.emp_no ORDER BY s.from_date DESC LIMIT 2) AS s2
