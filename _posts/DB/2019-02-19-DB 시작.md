...
![image1]({{ "/assets/DB/days01/image1.png" | absolute_url }}){: .shadow}  
<br><br>

C드라이브밑에 설치  

![image2]({{ "/assets/DB/days01/image2.png" | absolute_url }}){: .shadow}  
<br><br>

System database accounts(SYSDBA)의 비밀번호 설정  
가장 큰 권한을 가진 계정이다.  
비번 “ss123$” 로 설정   
![image3]({{ "/assets/DB/days01/image3.png" | absolute_url }}){: .shadow}  
<br><br>

![image4]({{ "/assets/DB/days01/image4.png" | absolute_url }}){: .shadow}  
...
<br><br>
설치 끝!  
![image5]({{ "/assets/DB/days01/image5.png" | absolute_url }}){: .shadow}  
...
![image6]({{ "/assets/DB/days01/image6.png" | absolute_url }}){: .shadow}  
...
![image7]({{ "/assets/DB/days01/image7.png" | absolute_url }}){: .shadow}    
![image8]({{ "/assets/DB/days01/image8.png" | absolute_url }}){: .shadow}    
![image9]({{ "/assets/DB/days01/image9.png" | absolute_url }}){: .shadow}    
...
![image13]({{ "/assets/DB/days01/image13.png" | absolute_url }}){: .shadow}    


테스트 버튼 클릭 시 SYS계정은 SYSDBA로 연결하라고 한다.  
롤에서 기본값 말고 SYSDBA로 바꿔 줘야함.  
![image14]({{ "/assets/DB/days01/image14.png" | absolute_url }}){: .shadow}    
<br>

저장하고 접속하자.
![image15]({{ "/assets/DB/days01/image15.png" | absolute_url }}){: .shadow}    
지정한 빨간색 테두리가 생기면서 접속...
<br>

접속하고 질의 작성기 워크시트에 SQL를 쓰고 실행.
![image16]({{ "/assets/DB/days01/image16.png" | absolute_url }}){: .shadow}    

SQL은 대소문자 구분을 하지 않기 때문에 `select * from TABS;` 로 사용해도 상관없지만
질의는 대문자, 테이블은 소문자로 쓰는 걸 권장한다.  

`SELECT * FROM all_users;` 실행  
![image17]({{ "/assets/DB/days01/image17.png" | absolute_url }}){: .shadow}    
...
![image18]({{ "/assets/DB/days01/image18.png" | absolute_url }}){: .shadow}    

만든 계정으로 새 접속을 만들어보자  
![image19]({{ "/assets/DB/days01/image19.png" | absolute_url }}){: .shadow}  
...
![image20]({{ "/assets/DB/days01/image20.png" | absolute_url }}){: .shadow}    

접속 성공


이번엔 연습용 계정인 HR에 로그인 해보자
![image21]({{ "/assets/DB/days01/image21.png" | absolute_url }}){: .shadow}    
...
![image22]({{ "/assets/DB/days01/image22.png" | absolute_url }}){: .shadow}    
...
![image23]({{ "/assets/DB/days01/image23.png" | absolute_url }}){: .shadow}    
...
WHERE USERNAME='HR';
```
![image24]({{ "/assets/DB/days01/image24.png" | absolute_url }}){: .shadow}    
...
![image25]({{ "/assets/DB/days01/image25.png" | absolute_url }}){: .shadow}    


그럼 sqlplus에서 이 sql파일을 통째로 실행시키자.

SCOTT계정으로 로그인.
![image26]({{ "/assets/DB/days01/image26.png" | absolute_url }}){: .shadow}  

@를 앞에 붙이고 sql파일 경로를 붙여넣기
![image27]({{ "/assets/DB/days01/image27.png" | absolute_url }}){: .shadow}  
...
`SELECT * FROM tabs;`
4 개의 table이 생겼다!
![image28]({{ "/assets/DB/days01/image28.png" | absolute_url }}){: .shadow}  
...
DESC INSA; 로 줄일 수 있음  
![image29]({{ "/assets/DB/days01/image29.png" | absolute_url }}){: .shadow}  
...
FROM INSA;
```
![image30]({{ "/assets/DB/days01/image30.png" | absolute_url }}){: .shadow}  
INSA의 모든 칼럼을 출력한다.
...
FROM INSA;
```
![image31]({{ "/assets/DB/days01/image31.png" | absolute_url }}){: .shadow}  
...
FROM INSA;
```
![image32]({{ "/assets/DB/days01/image32.png" | absolute_url }}){: .shadow}  
...
FROM INSA;
```
![image33]({{ "/assets/DB/days01/image33.png" | absolute_url }}){: .shadow}  
...
FROM EMPLOYEES;
```
![image34]({{ "/assets/DB/days01/image34.png" | absolute_url }}){: .shadow}  
