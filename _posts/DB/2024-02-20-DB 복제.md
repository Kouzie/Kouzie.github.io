...
`2PC` 처럼 동작하도록 지원하는 표준 기술이다.  
`XA 트랜잭션(eXtended Architecture)` 는 메세지 브로커와 DB같이 이종 서비스간 혹은 샤딩과 같이 물리적으로 분리된 데이터노드의 동작을 하나의 트랜잭션으로 묶어 데이터의 일관성을 보장한다.  

즉 2개의 별개의 시스템을 `2PC` 처럼 묶어서 `결과적으로 정확히 한번(effectively exactly once)` 동작하도록 할 수 있다.  

`Java Transaction API(JDBC 구현체)`, `JMS(브로커용 드라이버)` 를 사용하여 `XA 트랜잭션`을 구현할 수 있다.  
...
`합의 알고리즘`을 제공하는 대표적인 서비스는 아래와 같다.  

- Zookeeper(Zab 합의 알고리즘)  
- etcd(Raft 합의 알고리즘)  
...
`원자적 브로드캐스트`를 지원하는 `합의 알고리즘` 을 통해 값의 안정성과 일관성을 관리하며 `Hadoop, HBase, Kafka, Kubernetes` 등 분산 데이터 처리 시스템의 코디네이션 및 관리에 위와같은 서비스가 사용되는 이유가 `합의 알고리즘` 때문이다.  

#### Zookeeper(Zab) 

`Zookeeper` 와 `Zab(ZooKeeper Atomic Broadcast)` 원자적 브로드캐스트를 지원하는 합의 알고리즘에 대해 알아본다.  

![1](/assets/DB/mysql/db_replica15.png)  

그림과 같이 1개의 Leader, N개의 Follower로 구성되어 있고 

> 위와 같은 구조를 주키퍼 앙상블(Zookeeper Ensemble) 이라 부르기도 함.  

내부의 데이터는 `Zab` 알고리즘을 통해 공유된다.  

Request Processor에서 처리한 요청을 트랜잭션을 생성하여 모든 서버에게 전파한다.  
[Leader-Propose] -> [Follower-Accept] -> [Leader-Commit] 단계로 구성된다.

`DB Cluster` 에서 `leader replica` 가 오류로 죽은 경우 모든 `read replica` 들이 자신이 새로운 `leader` 가 되곘다고 요청할 수 있다.  
이런 요청을 Zookeeper 가 받아 Zab 을 통해 원자적으로 처리하고 새로운 `leader replica` 를 공유할 수 있다.   
