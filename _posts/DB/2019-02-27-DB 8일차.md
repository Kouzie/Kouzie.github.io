...
하나의 타입을 만드는데 안에 여러개의 정보를 가지고 있는 테이블 같은 데이터 타입.

CREATE TYPE people_tab_typ AS TABLE OF people_typ
그리고 만든 타입에 앞으로 쓸 자료형 이름을 부여... 이제 만든 타입을 쓸 수 있다.
...
1. Partitioned Tables & Indexes 생성
...
PARTITION BY RANGE(age)
  (partition Part1 values less than (20) tablespace Part01,
   partition Part2 values less than (30) tablespace Part02,
   partition Part3 values less than (40) tablespace Part03,
   partition Part4 values less than (50) tablespace Part04,
   partition Part5 values less than (MAXVALUE) tablespace Part05);
...
21∼30세의 age는 part2라는 파티션으로 저장되고 그 테이블스페이스는 Part02이다.마지막 MAXVALUE라는 키워드는 50세 이후의 age를 갖는 모든 데이터가 Part5라는 파티션에 저장된다.
...
캐싱 테이블은 빈번하게 사용되는 테이블 데이터를 데이터버퍼 캐시영역에 상주시켜 검색시 성능을 향상시킴. 
...
PCTFREE 	BLOCK에 data를 INSERT할 때 INSERT를 하지 않고 남겨 놓은 free space에 대한 비율을 나타내는 parameter
PCTUSED 	INSERT가 시작되는 시점을 결정하기 위한 parameter
INITRANS 	BLOCK을 구성할 때 BLOCK의 header에다가 transaction enrty를 몇개를 만들 것인가를 결정하는 parameter
MAXTRANS 	BLOCK 하나가 동시에 지원하는 transaction의 최대 수를 지정하는 parameter
STORAGE절 	INITIAL, NEXT, PCTINCREASE, MAXEXTENTS등을 지정할 수 있는 절
INITIAL 	첫 번째 EXTENT의 크기
NEXT 	다음 EXTENT의 크기
MINEXTENTS 	처음 생성된 EXTENT의 총 개수
MAXEXTENTS 	할당 가능한 EXTENT의 총 개수
PCTINCREASE 	다음 EXTENT의 크기 증가율
FREELISTS 	사용 가능한 BLOCK의 LIST 수
	

LOGGING/NOLOGGING 	Redo Log File에 TABLE 생성과 관련된 정보의 저장 여부를 결정하는 parameter (default = NOLOGGING)
CACHE/NOCACHE 	Full table scan시 읽어들인 BLOCK을 buffer cache 내에 지정할 지 여부를 나타내는 parameter
...
ALTER TABLE... RENAME COLUMN 컬럼 TO 컬럼
ALTER TABLE... DROP[CONSTRAINT] 제약조건 
ALTER TABLE... DROP COLUME컬럼

이 문을 실행하려면 자신의 스키마에 대해 ALTER 객체 권한과 ALTER ANY TABLE 시스템 권한이 있어야 한다.
...
MODIFY 유의사항…
컬럼 크기의 변경은 저장된 데이터의 크기보다 크거나 같을 경우에만 가능하다.
...
rollback;
commit;
롤백, 커밋으로 뒤로가기 가능, DML의 변경내용은 세션에만 저장되기 때문
...
1) 자신의 테이블에서 subquery하여 UPDATE
 subquery를 사용하면 한 번의 update 명령으로 여러 개의 컬럼을 수정할 수 있다. 여러 컬럼을 subquery의 결과 값으로 UPDATE 한다.
...
두 경우 모두 같은 유형, 같은 개수의 칼럼을 가져야 한다.
...
regdate는 default로 sysdate를 받는다.
 

INSERT또한 DELETE나 UPDATE처럼 추가했으면 롤백이나 커밋이 가능하다.

괄호안 컬럼명 순서대로 값을 부여해야 오류가 안뜬다! 컬럼의 순서는 바뀌어도 상관없다.

 
INSERT INTO 테이블명 VALUES (값…)

기본키인 ID가 같은 값인 데이터를 INSERT!

INSERT INTO tbl_member
VALUES ('admin', '김길동', 27, '19931029', sysdate, '010-1234-1234', null);

오류 발생!
ORA-00001: unique constraint (SCOTT.SYS_C007072) violated

PRIMARY KEY 제약조건에 NOTNULL, UNIQUE 제약조건이 포함 되어있다.
이런 유니크(유일성) 제약조건을 줄여 UK제약조건이라 하고 모든 제약조건에는
SCOTT.SYS_C007072과 같은 코드명이 자동 부여된다.

참고로 기본키와 NOT NULL 제야조건을 같이 주는 이유는 기본키 제약조건이 사라지면 NOT NULL은 남겨놓기 위해서…

INSERT INTO에서 컬럼명을 생략하고 싶다면 모든 칼럼에 해당하는 값을 입력해야 한다.
...
conditional FIRST INSERT	조건에 맞는 행들을 원하는 여러 테이블에 나누어 삽입
(첫 when조건이 충족되면 다음의 when절은 수행하지 않음)
...
WHEN과 THEN, ELSE 조건절을 사용해서 INSERT할 ROW를 분류 가능.
...
pivoting insert 문은 이와 같이 비관계형 데이터베이스 구조의 컬럼을 읽어서 관계형 데이터베이스 구조의 테이블에 데이터를 일괄 입력시킬 때 사용한다.
...
상품A는 UPDATE로 더하는 수정작업을, 상품C는 INSERT로 추가해야 한다.
...
dept1에 dept2를 MERGE해보자, deptno을 기준으로 같으면 dname과 loc을 서로 같게 UPDATE하고
다르면 INSERT 시키자

UPDATE연산은 4번, INSERT연산은 1번 총 5개행이 변경된다.
  
병합되어서 dept1와 dept2가 서로 같게 되었다. 물론 WHEN MATCHED THEN과 WHEN NOT MATCHED THEN을 통해
서로 다르게 설정할 수 있다.
