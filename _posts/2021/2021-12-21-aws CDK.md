...
<!-- 
1. 구성(또는 초기화)
코드는 정의된 모든 구성을 인스턴스화한 다음 함께 연결합니다. 이 단계에서 모든 구성(앱, 스택 및 해당 하위 구성)이 인스턴스화되고 생성자 체인이 실행됩니다. 대부분의 앱 코드는 이 단계에서 실행됩니다.

2. 준비
prepare메서드를 구현한 모든 구문 은 최종 수정 라운드에 참여하여 최종 상태를 설정합니다. 준비 단계는 자동으로 발생합니다. 사용자는 이 단계에서 피드백을 볼 수 없습니다. "prepare" 후크를 사용해야 하는 경우는 드물며 일반적으로 권장되지 않습니다. 작업 순서가 동작에 영향을 줄 수 있으므로 이 단계에서 구성 트리를 변경할 때 매우 주의해야 합니다.

3. 검증
구현한 모든 구성 validate메서드는 올바르게 배포될 상태에 있는지 확인하기 위해 자체 유효성을 검사할 수 있습니다. 이 단계에서 발생하는 모든 유효성 검사 실패에 대한 알림을 받게 됩니다. 일반적으로 가능한 한 빨리(일반적으로 일부 입력을 받는 즉시) 유효성 검사를 수행하고 가능한 한 빨리 예외를 throw하는 것이 좋습니다. 유효성 검사를 조기에 수행하면 스택 추적이 더 정확해지기 때문에 진단 가능성이 향상되고 코드가 계속 안전하게 실행될 수 있습니다.

4. 합성
이것은 AWS CDK 앱 실행의 마지막 단계입니다. 에 대한 호출에 의해 트리거되고 app.synth()구성 트리를 탐색하고 synthesize모든 구성 에서 메서드를 호출합니다 . 구현 synthesize하는 구성은 합성에 참여하고 결과 클라우드 어셈블리에 배포 아티팩트를 내보낼 수 있습니다. 이러한 아티팩트에는 AWS CloudFormation 템플릿, AWS Lambda 애플리케이션 번들, 파일 및 Docker 이미지 자산, 기타 배포 아티팩트가 포함됩니다. 클라우드 어셈블리 는 이 단계의 출력을 설명합니다. 대부분의 경우 synthesize메서드 를 구현할 필요가 없습니다.

5. 배포
이 단계에서 AWS CDK Toolkit은 합성 단계에서 생성된 배포 아티팩트 클라우드 어셈블리를 가져와 AWS 환경에 배포합니다. 자산을 Amazon S3 및 Amazon ECR 또는 어디로 가야 하는지에 업로드한 다음 AWS CloudFormation 배포를 시작하여 애플리케이션을 배포하고 리소스를 생성합니다.
 -->
...
CDK 를 사용해 생성할 `Stack` 과 이미 프로비저닝된 AWS 리소스들을 가져오는데 사용하는 개념   

`cdk.json` 파일에 `context` 에 대한 정보가 저장되며 
...
`cdk syth` 와 같은 명령 사용지 `-c` 옵션을 통해 `context` 에 어떤 정보를 넘길지 `key=value` 형식으로 전달  
