...
피포탈에서 만든 리엑티브 라이브러리, 마찬가지로 `Reactive Stream` 스펙을 따르며 사용법이 `RxJava` 와 굉장히 유사하다.  
...
데이터가 처음 발행되면 수신받은 직후 `cancel` 로 더이상을 발행을 종료한다.  


![image20](/assets/java/reactor/image20.svg)  

`complete` 될 때 마지막 발행 데이터만 수신받거나 데이터가 발행되지 않은경우 `null` 데이터 수신  
