...
기존의 `Blocking` 모델의 경우 `listen(), connect(), accept(), recv(), send(), read(), write(), recvfrom(), sento(), close()` 등의 함수는 `커널 I/O 버퍼 -> 유저 I/O 버퍼`로 복사 후 이용해야 하기에 
복사완료될때 까지 스레드가 봉쇄, 시스템 콜이 발생할 수 있다. 
