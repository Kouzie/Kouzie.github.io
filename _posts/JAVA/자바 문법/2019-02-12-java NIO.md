...
고전적인 **비동기 프로그래밍** 방식으로 스레드를 여러개 만들어 동시에 수행하는 방법이 있다.  
하지만 스레드가 들어나면 그만은 컨텍스트 스위칭이 일어나게 되고 임계영역이나 공유데이터에 대한 동시성 접근도 제어해야 한다.  

때문에 하나의 스레드가 동시에 여러개의 작업을 수행하는 NIO 방법을 사용해 비동기 프로그래밍 많이한다.  
I/O 처리를 수행해야할 때 스레드가 `block & wakeup` 되지않고 바로 다른작업을 하러 가능 방식이다.  

이런 특징 때문에 비동기

| Block IO                  | Non Block IO                     |
| --------------------- | ------------------------- |
