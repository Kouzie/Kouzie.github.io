...
application에서 가장 많이 사용되는 기능으로  
실제 서버에 저장된 파일을 읽어오거나 저장할 때 정확한 위치를 통해 File객체와 입출력 스트림을 생성해야 한다.  
...
예외가 발생할만한 페이지에 예외처리를 위한  **디렉티브(지시자)**를 추가하자  
