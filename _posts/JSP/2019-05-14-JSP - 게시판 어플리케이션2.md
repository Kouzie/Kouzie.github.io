...
```
![image53]({{ "/assets/jsp/image44.png" | absolute_url }}){: .shadow}     
...
```
![image52]({{ "/assets/jsp/image44.png" | absolute_url }}){: .shadow}     
...
따라서 `ListArticleHandler`정의도 몇줄 되지 않는다.  
조회할 게시글 넘버만 파라미터로 전달받아 DB에서 `SELECT`한 뒤 View페이지로 출력한다.  
```java
...
우선 `ArticleDao`에 `selectById()`, `increaseReadCount()`메서드를 정의하자.  
...
![image56]({{ "/assets/jsp/image56.png" | absolute_url }}){: .shadow}     
