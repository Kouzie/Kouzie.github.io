...
`state` 값은 `mutations` 메서드로만 변경가능하다.  
...
getters 에는 메서드가 아닌 필드와 화살표함수로 값을 정의하며  
`state`, `getters`, `rootState`, `rootGetters` 를 인자로 받을 수 있다.  
...
`vuex store` 의 `state`, `mutations`, `actions`, `getter` 에 접근하기 위해 `store` 를 직접 사용했는데  
`vue` 인스턴스 내부에서 `store` 객체에 직접접근을 권장하지 않는다.  

아래처럼 매칭되는 헬퍼메서드 사용을 권장한다.  
