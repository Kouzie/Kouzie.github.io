...
다음과 같이 `지역 vue 컴포넌트` 를 생성하면 컴포넌트 내부의 html 요소들을 모두 무시된다.  
...
내부의 요소를 출력하고 싶다면 `인라인 템플릿` 혹은 별도의 `컴포넌트 템플릿`을 생성해서 상위 컴포넌트에 포함될 수 있도록 해야한다.  

슬롯을 사용하면 간단하다.  

다음과 같이 `my-component` `vue 컴포넌트`에 인라인 템플릿을 정의하여 컴포넌트를 정의

그리고 `컴포넌트 템플릿` 내부에는 여러 html 요소들을 추가한다.  
...
아래와 같이 슬롯에 `name` 속성을 사용해 원하는 슬롯 위치에 상위 컴포넌트에서 정의한 html 요소를 집어넣을 수 있다.  

상위 컴포넌트에선 집어넣고 싶은 루트 html 요소에 `slot` 속성을 지정한다.  
...
하위 컴포넌트 `child` 내부에 또 `<template>` 태그가 있다.  

`<template>` 태그 안의 `scope=p1` 으로 설정후 자식에게 전달한다.  
