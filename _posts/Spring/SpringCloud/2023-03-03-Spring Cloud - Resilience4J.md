...
`TimeLimiter + Retry`: 느린 서비스가 일시적으로 실패할 때 재시도
`CircuitBreaker + Fallback`: 계속 장애 시 차단 후 대체 로직
`Bulkhead + RateLimiter`: 리소스 보호 + 과도한 요청 제한
...
MSA 환경의 깊숙한 서비스에서 에러가 발생할 경우 해당 에러는 다시 최 전방으로 응답된다.  
그 사이의 모든 서비스들이 4XX, 5XX 에러로그가 출력되고 에러를 응답하게 된다.  

하나의 서비스 장애가 결국 다른 서비스로의 장애 전파로 이뤄지기 때문에 서비스의 장애가 발견되면 개발자의 장애 대응 로직으로 전환될수 있도록 해야한다.  
