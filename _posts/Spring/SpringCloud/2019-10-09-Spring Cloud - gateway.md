...
> 하지만 `zuul` 은 `Spring Cloud Hoxton Release` 를 마지막으로 더이상 지원하지 않음.
...
- **route**
  게이트 웨이 기본 요소 목적지 url 과 각종 조건 설정  
- **predicates**
  조건자, 각 요청 처리전 실행되는 로직, `java8` `Predicate` 인터페이스 기반 전처리 조건 설정  
- **filters**
  필터, 각 요청/응답 값의 필터 설정, `GatewayFilter` 의 구현체  
