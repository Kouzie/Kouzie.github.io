...
public class MyController {
    ...
    @Value("${myapp.default-page-size}")
    private int defaultPageSize;

    @GetMapping("/example")
    public String getExample(
            @RequestParam(name = "pageSize", defaultValue = "#{${myapp.default-page-size}}") int pageSize) {
        return "Page size: " + pageSize;
    }
}
```

@... 을 사용하면 bean 으로 등록되어 있는 configuration 에서 값을 가져와 사용할 수 도 있다.  
...
public class MyController {
    ...
    @GetMapping("/example")
    public String getExample(
            @RequestParam(name = "pageSize", defaultValue = "#{@myAppProperties.defaultPageSize}") int pageSize) {
        return "Page size: " + pageSize + ", App name: " + properties.getAppName();
    }
}
```
