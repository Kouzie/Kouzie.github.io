...
당연히 `FetchType.EAGER` 를 사용하면 부가적인 코드를 생략할 수 있더 코드가 짧아지지만  
...
하지만 기존 쿼리 메서드 `findAllBy()` 로 읽어올 경우 `JOIN`을 사용하지 않기때문에 `Board List`를 얻는 쿼리를 호출하고 각 Board에 대한 댓글 개수를 읽어오는 쿼리를 수행한다.  
