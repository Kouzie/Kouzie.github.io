...
스프링 프레임웤은 이제는 자바 웹 개발자라면 반드시 알아야 할 거의 표준 프레임 워크이다.  
...
`spring-jms` | `Java Message Service` 서버(`ActiveMQ`, ₩ 등)와 메시지를 쉽게 주고 받을 수 있도록 하기 위한 템플릿(`annotation` 등) 제공한다.  
...
지금은 간단히 `Spring Framework`에서 객체 생성방법을 배우는 목적으로 그냥 `java` 프로젝트에 프레임워크에 필요한 `jar` 를 수동으로 추가해서 객체를 생성해보자.  
