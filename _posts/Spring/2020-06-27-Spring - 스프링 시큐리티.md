...
```
자동으로 사용할 라이브러리의 네임스페이스가 추가된 xml파일이 만들어진다.  
...
`<security:http>` - 인증, 인가를 설정하는 태그  
`<security:intercept>` - 인증이 필요한 페이지를 등록하고 해당하는 권한을 설정  
...
`<security:intercept-url>`태그 속성을 통해 각페이지를 다음과 같이 설정한다.
...
`<security:http>`태그의 `auto-config="true"` 속성이 사라지고 `<form-login>`태그의 `login-page`속성을 통해 로그인페이지를 지정한다.  
...
사용자가 로그인 했다면 `Principal`클래스의 `userPrincipal`객체가 존재할 것.  
