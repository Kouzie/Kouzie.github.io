...
고전파는 공유 의존성에 대해 대역을 새우고  
런펀파는 모든 헙력자에 대해 대역을 세운다  

unit test 에서 대역에 사용하는 단어는 `[mock, stub]` 2가지로 나뉜다.  

- **mock**: 외부로 데이터를 송신하는 공유 의존성을 모방
- **stub**: 내부로 데이터를 수신하는 공유 의존성을 모방  

`CQS(command query separation)` 를 예로 들면  

- `Command` 를 수행하는 의존성은 `mock`  
- `Query` 를 수행하는 의존성은 `stub`  

Command 와 Query 를 모두 수행하는 협력자는 mock 으로 취급한다.  
