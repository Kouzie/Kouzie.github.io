...
> 테스트에서 **회귀(regression)** 는 테스트 코드는 정상동작 하더라도 제품코드가 비정상인 `False Negative` 상황을 뜻한다.  
...
캡슐화가 되어있을 수록 `Mock` 객체를 사용하게 되다보니 실행시키는 제품코드수가 줄어들기에 `회귀방지`와 `리펙터링 내성`은 적대적 관계이다.  
...
프로젝트 규모가 클수록 협력자와 공유 의존성을 많아지기에 통합 테스트 또한 단위 테스트만큼 중요해진다.  
