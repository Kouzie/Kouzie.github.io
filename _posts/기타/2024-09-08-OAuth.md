...
> 사용자가 자신의 자격 증명을 제공하지 않고 제3자 애플리케이션에 자신이 소유한 자원의 제한적인 접근 권한을 부여할 수 있는 표준 프로토콜  
> 
> 참고  
...
각종 캘린더, 사진첩 등 Google Drive 를 사용하는 서드파티 앱 사용시 많이 활용하는 프로토콜.  
앱에 Google Drive 계정을 넘기지 않고 Google Drive 에 접근을 허가시킬 수 있음.  
...
`Grant Type` 은 `Resource Client` 의 종류에 따라서 사용여부가 갈리는데, `Resource Client` 종류는 아래와 같이 나뉜다.  
...
`Authorization Code` 지원 `Resource Client` 웹페이지에서 사용자가 `OAuth` 로그인 버틀을 누르면 아래와 같은 URL 로 로그인 모달창이 열린다.   
...
`ID Token` 내부에는 아래와 같이 `JWT Claims` 형태로 OIDC 가 지원하는 신원데이터 형식이다.  
`ID Token` 은 JWT 형식의 토큰으로 HTTP 헤더의 최대 사이즈인 4KB 이내로 저장가능.  
