...
전동적인 계층형 아키텍처는 아래와 같이 요청 흐름을 간결하게 구조가 장점인 아키텍처이다.  
...
시간이 지나면서 기존 서비스 코드가 의존하는 영속성 계층의 코드가 많아지고  
기존 코드에 유스케이스들이 추가되면서 비대한 서비스 클래스가 생성된다.  

![image02](/assets/기타/clean_arch4.png)  

서비스를 테스트하기도 어려워지고 유스케이스를 책임지는 서비스를 찾기도 어려워진다

클린 아키텍처는 이러한 계층형 아키텍처에서 발생하는 각종 문제점들을 막기 위해  
여러가지 강제 원칙들을 넣은 아키텍처라 할 수 있다.  
...
계층형 서버개발을 자주하였고 `Java Persistence API(JPA)` 를 사용중이라면 `[Entity, Repository]` 가 강하게 결합되어,  
위 그림처럼 `DB(Persistence) 레이어` 와 `Entity(도메인) 레이어` 를 분리해서 생각하기가 어렵다.  

클린 아키텍처는 둘 사이에 `DIP` 개념으로 `도메인 레이어` 와 `DB 레이어` 를 분리하고,  
도메인 레이어에서 사용할 **도메인객체**, 영속성 레이어에서 사용할 **엔티티객체**를 따로 정의하고 매핑을 통해 데이터를 시키는 방식을 사용한다.  
...
문서로 정립된 클린 아키텍처의 원칙을 어느정도 코드와 패키지 구조로 표기한 아키텍처.  

아래 헥사고날 아키텍처 그림을 보면 클린 아키텍처의 원칙을 지키기 위한 DIP를 위한 패키지 구조, 생성해야할 클래스들을 구체적으로 표현한다.  
...
그리고 각 레이어간 의존성은 `DIP` 를 최대한 활용하기 위해 `interface` 를 사용해야 한다.  

`input port` 어뎁터는 외부에서 들어오기 때문에 어플리케이션을 **주도하는 어뎁터**라 표현하고,  
`output port` 는 어플리케이션이 호출하기 때문에 **주도되는 어뎁터**라 표현한다.  
이런 특징때문에 `포트와 어뎁터(ports-and-adapters)패턴`이라 부르기도 한다.  

유스케이스는 `DIP` 로인해 `[input port, output port]` 의존성이 완벽하게 분리되어 있기 때문에,  
유스케이스는 DB 의 `output port` 가 `[JPA, JDBC, jooq, MyBatis, NoSQL]` 중 어떤 구조를 사용하는지 알 필요가 없고,  
외부 `input port` 가 `[HTTP, gRPC, Messaging]` 중 어떤 입력 방식을 사용하는지 알 필요가 없다.  
...
`adapter` 패키지에 외부시스템에 대한 `[input port, output port]` 어뎁터를 정의하고,  

`application.port` 패키지에 유스케이스에 대한 `[input port, output port]` 어뎁터를 정의한다.  
...
클린 아키텍처에선 어플리케이션 코드들이 `JPA`에 관련해서 모르는 상태로 만들어야 한다.  

`영속성 어뎁터` 의존성을 다른 클래스들이 갖지 못하도록 DIP 방식을 사용한다.  
...
`웹 어뎁터` 와 마찬가지로 의존성 분리를 위해 `applicaiton.port.out`에 정의된 `영속성 어뎁터`는 최대한 유즈케이스별로 쪼개는것을 권장한다.  
...
`어뎁터 레이어`와 `어플리케이션 레이어`는 항상 이동시에 `[input port, output port]` `DIP` 인터페이스를 통해서만 호출하고 호출된다.  

레이어간 이동시에 유스케이스에 따라 적절한 데이터 매핑 방식을 선택한다.  
...
`DIP` 로 인해 생성된 `interface` 외에는 패키지 밖의 객체간 의존성이 발생할 일이 없다.  
그럼으로 `DIP` 로 생성되는 클래스를 제외한 모든 메서드 접근제한자를 `default(pakcage-private)` 으로 설정하는 것을 권장한다.  
