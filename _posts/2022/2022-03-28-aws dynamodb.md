...
기본테이블의 `파티션 키` 및 `정렬 키` 와는 아예 다른 필드를 사용하여 인덱스 생성시 GSI 사용
즉 사용하는 `파티션 키` 및 `정렬 키` 가 기본테이블과 다른 경우  

**2. Local Secondary Index(LSI)**
기본테이블의 `파티션 키` 는 생성된 인데스의 `파티션 키` 와 동일하지만 `정렬 키`가 다른 경우  
